...
def main():
    ...
    while True:
        ...
        if ret:
            frame_resized = cv2.resize(frame, (832,832)) # resize frame for prediction       
        else:
            break
        ...
        # Detecting objects
        blob = cv2.dnn.blobFromImage(frame_resized, 0.00392, (832, 832), (0, 0, 0), True, crop=False)
        ...
        for i in range(len(boxes)):
            if i in indexes:
                ...
                for k in range (len(centroids)):
                    c = centroids[k]
                    dist=(get_distance(c[0],centroid[0],c[1],centroid[1]))
                    di =( dist / 68.5 ) * 6
                    D = "{:.{}f}ft".format( di, 2 )
                    #D= f'{str(di)} ft'
                    ...
                    if get_distance(c[0],centroid[0],c[1],centroid[1]) <= distance:
                        ...
                        cv2.putText(frame, D,( x, y ) , cv2.FONT_HERSHEY_COMPLEX ,0.5, RED, 1)
        ...
        frame_2 = cv2.resize(frame,(1000,1000))
