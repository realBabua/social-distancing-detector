            ...
            if i in indexes:
                ...
                for k in range (len(centroids)):
                    ...
                    if get_distance(c[0],centroid[0],c[1],centroid[1]) <= distance:
                        box_colors[k] = 1
                        my_color = 1
                        xa = int((int(c[0])+int(centroid[0]))/2)
                        xb = int((int(c[1])+int(centroid[1]))/2)
                        cv2.line(frame, (int(c[0]),int(c[1])), (int(centroid[0]),int(centroid[1])), YELLOW, 1,cv2.LINE_AA)
                        cv2.putText(frame, D,( xa, xb ) , cv2.FONT_HERSHEY_COMPLEX ,0.4, RED, 1)
